...
test(
  ...
  (t, length: number, dirpath: string) => {
    // tslint:disable-next-line:no-string-literal
...
test(
  'Get entry name',
  (t, length: number, filename: string) => {
    // tslint:disable-next-line:no-string-literal
...
test(
  ...
  (t, length: number, filename: string) => {
    ...
    const desc: IZipEntryDescription = entry.describe();
...
test(
  'Decompress entry',
  async (t, length: number, filename: string, fileContent: string) => {
    // tslint:disable-next-line:no-string-literal
    const entries: ZipEntry[] = t.context['zip'].findEntries([filename]);
    t.is(entries.length, length);
    const entry = entries[0];
